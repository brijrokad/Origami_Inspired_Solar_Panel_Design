...
def getFitness(individ, numConflicts, l, w, sapm, cec, df, latitude, longitude, currFitCount, res=6, conflictDeduction=50):
    ...
    for face in FaceList:
        ...
        if face_position >= 47:
            print("indAngles: " + str(indAngles))
            print("face_position: " + str(face_position))
...
def mutationGuassianList(individual, pm, sigmaList, l, w, maxCuts, zMin, zMax, tMin, tMax, aMin, aMax, res=6, 
                         zThresh=20, tThresh=90, aThresh=45):
    """
    MAKES A POPULATION
    inputs- individual - list of ints - an individual. It looks like: [bit1 bit2 bit2 ... z1, t1, a1, z2, t2, a2...]
            pm - float between 0.0 and 1.0 - the probability a mutation will occur for an individual
            sigma- list of floats - standard deviation for the normal distribution where its length is equal to the length
...
def steadyStateGA(panelParams, popSize, pm, numMutations, fitnessCountLimit, attempts, checkIteration, conflictDeduction, toPrint=True):
    ...
    for i in range(attempts): 
        ...
        while currFitCount <= fitnessCountLimit:
            ...
            child1 = bitStringResolution(child1, bitStringLen, maxCuts)
            child2 = bitStringResolution(child2, bitStringLen, maxCuts)
            ##survival selection
...
--------------------------------------------------------------------------------------
"""
def createPopSigmas(popSize):
    """
    CREATES THE LIST OF SIGMA VALUES FOR THE INITIAL POPULAITON
    inputs - popSize - int - the size of the population
    outputs - popSigmas - list - the list of floats the size of the population
    """

    #initialzation of the population's sigma list
    popSigmas = []

    #giving individuals their own sigmas
    for i in range(popSize):
        
        indSigma = abs(np.random.normal(0,1))
        popSigmas.append(indSigma)
    
    return popSigmas


def recombination(parent1, parent1Sigma, parent2, parent2Sigma, l, w, maxCuts, res=6):
    ...
    randInt = np.random.randint(1)
    if randInt == 0:
        childSigma = parent1Sigma
    else:
        childSigma = parent2Sigma

    bitStringLen = int((l-1)/res) + int((w-1)/res)
    child = bitStringResolution(child, bitStringLen, maxCuts)
    return child, childSigma


def mutationES(child, childSigma, childFitness, popSize, l, w, maxCuts, zMin, zMax, tMin, tMax, aMin, aMax, res=6):

    bitStringLen = int((l-1)/res) + int((w-1)/res)
    paramList = child[bitStringLen:]

    #overall learning rate
    tauPrime = 1/math.sqrt(2*popSize)

    #coordinate wise learning rate
    tau = 1/math.sqrt(2*math.sqrt(popSize))

    #newSigma intermediate calculations
    norm = np.random.normal(0,1)
    exp = tauPrime*np.random.normal(0,1) + tau*norm

    newSigma = childSigma*math.exp(exp)

    #Create new z, t, and a values for child
    for i in range(len(paramList)):

        if (i % 3) == 0:
            
            tempZ = paramList[i] + newSigma*norm
            if (tempZ < zMin) or (tempZ > zMax):
                tempZ = paramList[i] - newSigma*norm
            
            paramList[i] = tempZ
        elif (i % 3) == 1:
            tempT = paramList[i] + newSigma*norm
            if (tempT < tMin) or (tempT > tMax):
                tempT = paramList[i] - newSigma*norm
            
            paramList[i] = tempT
        elif (i % 3) == 2:
            tempA = paramList[i] + newSigma*norm
            if (tempA < aMin) or (tempA > aMax):
                tempA = paramList[i] - newSigma*norm
            
            paramList[i] = tempA

    newChild = child[:bitStringLen] + paramList

    #due bit resolution on individual
    newChild = bitStringResolution(newChild, bitStringLen, maxCuts)

    return newChild, newSigma
    

def getLambdaGeneration(population, popSigmas, popFitness, l, w, sapm, cec, df, latitude, longitude, currFitCount, maxCuts,
                     zMin, zMax, tMin, tMax, aMin, aMax, res=6, conflictDeductions=50, zThresh=20, tThresh=90, aThresh=45):
    ...
    lambdaSigma = []
    lambdaFitness = []
    for i in range(7*popSize):
        
        #grabbing parents and sigmas for recombination
        p1_randInt = np.random.randint(popSize)
        p2_randInt = np.random.randint(popSize)

        parent1 = population[p1_randInt]
        parent1Sigma = popSigmas[p1_randInt]
        

        parent2 = population[p2_randInt]
        parent2Sigma = popSigmas[p2_randInt]
        


        #recombination
        child, childSigma = recombination(parent1, parent1Sigma, parent2, parent2Sigma, l, w, maxCuts, res)
        numConflicts = conflictCount(child, l, w, zMin, zMax, tMin, tMax, aMin, aMax, res, zThresh, tThresh, aThresh)
        childFitness, currFitCount = getFitness(child, numConflicts, l, w, sapm, cec, df, latitude, longitude, currFitCount, res, conflictDeductions)
        
        #mutation
        newChild, newChildSigma = mutationES(child, childSigma, childFitness, popSize, l, w, maxCuts, 
                                    zMin, zMax, tMin, tMax, aMin, aMax, res)

        #geting number of conflicts for newChild
        numConflicts = conflictCount(newChild, l, w, zMin, zMax, tMin, tMax, aMin, aMax, res, zThresh, tThresh, aThresh)

        #computeing newChild Fitness
        newChildFitness, currFitCount = getFitness(newChild, numConflicts, l, w, sapm, cec, df, latitude, longitude, currFitCount, res, conflictDeductions)

        #adding child and it's sigma to their list
        if newChildFitness < childFitness:
            lambdaGeneration.append(child)
            lambdaSigma.append(childSigma)
            lambdaFitness.append(childFitness)
        else:
            lambdaGeneration.append(newChild)
            lambdaSigma.append(newChildSigma)
            lambdaFitness.append(newChildFitness)

    #returning the population, sigmas, and fitness for the lambda generation
    return lambdaGeneration, lambdaSigma, lambdaFitness, currFitCount

def surviorSelection(popSize, popList, popSigmas, popFitness):

    muFitness = nlargest(popSize, popFitness)
    muSigmas = []
    muPop = []
    for val in muFitness:
        index = popFitness.index(val)
        muSigmas.append(popSigmas[index])
        muPop.append(popList[index])

    return muPop, muSigmas, muFitness

def getBestES(pop, fitnessPop, sigmasPop):
    """
    inputs - pop - list of individuals - a list of all of the individuals
           - fitnessPop - list of floats - list of each indiviudal's fitness. The index in fitnessPop corresponds with the 
                index of pop.
            - sigmasPop - list of floats - list of each indidivual's sigma values
    outputs - list of ints - the best individual
            - floats - the best individuals sigma
            - float - the best individual's fitness 
    """
    #get max value
    maxFitness = max(fitnessPop)
    index = fitnessPop.index(maxFitness)
    bestIndividual = pop[index]
    bestIndSigma = sigmasPop[index]
    
    #return solutions
    return bestIndividual, bestIndSigma, maxFitness

def evolutionaryStrategies(panelParams, popSize, fitnessCountLimit, attempts, checkGeneration, conflictDeductions, toPrint=True, method=','):
    """
    inputs - panelParams - list of parameters - of form [l, w, maxCuts, res, zMin, zMax, tMin, tMax, aMin, 
                                                            aMax, zThresh, tThresh, aThresh]
           - popSize - int - size of a population
           ...
           - conflictDeductions - int - the amount that should be deduced from fitness if there is a conflict
           - toPrint - boolean -  if true, then results will be printed as function runs
           - method - str - ',' OR '+' depending on which survial method (mu,lambda) OR (mu + lambda)
    outputs - dfBest - pandas dataframe storing the best individual and their fitnesses
            - dfAvg - pandas dataframe storing the average fitness and the associated iteration for each attempt
    """
    l = panelParams[0]
    ...
    #initialize the list of best fitnesses and individuals and sigmas
    bestIndList = []
    bestFitnessList = []
    bestSigmaList = []
    ...
    for i in range(attempts): 
        ...
        popFitness, currFitCount = getPopFitness(population, l, w, zMin, zMax, tMin, tMax, aMin, aMax, currFitCount, 
                                    sapm, cec, df, latitude, longitude, res, zThresh, tThresh, aThresh, conflictDeductions)
        ...
        #initialize the best individual and fitness and sigma
        bestGlobalInd = []
        bestGlobalFitness = 0.0
        bestGlobalSigma = 0.0
        ...
        while currFitCount < fitnessCountLimit:
            ...
            #making Lambda generation
            lambdaPop, lambdaSigma, lambdaFitness, currFitCount = getLambdaGeneration(population, popSigmas, popFitness, 
                                                            l, w, sapm, cec, df, latitude, longitude, currFitCount, 
                                                            maxCuts, zMin, zMax, tMin, tMax, aMin, aMax, res, 
                                                            conflictDeductions, zThresh, tThresh, aThresh)
            
            if method == ',':
                population, popSigmas, popFitness = surviorSelection(popSize, lambdaPop, lambdaSigma, lambdaFitness)
            elif method == '+':
                lambdaPop = population + lambdaPop
                lambdaSigma = popSigmas + lambdaSigma
                lambdaFitness = popFitness + lambdaFitness
            ...
            if (generation % checkGeneration) == 0:
                ...
                avgFitness = getAvgPopFitness(popFitness)
                
                bestInd, bestSigma, bestFitness = getBestES(population, popFitness, popSigmas)
                if toPrint == True:
                    print("generation " + str(generation))
                    print("current average fitness: " + str(avgFitness))
                    print("  ")
        ...
        bestIndList.append(bestGlobalInd)
        bestFitnessList.append(bestGlobalFitness)
        bestSigmaList.append(bestGlobalSigma)
        listOfAvgFitnessLists.append(avgFitnessList)
    ...
    bestDf['best individuals'] = bestIndList
    bestDf['best Fitnesses'] = bestFitnessList
    bestDf['best Sigmas'] = bestSigmaList
