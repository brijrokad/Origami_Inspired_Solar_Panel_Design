...
def recombination(parent1, parent1Sigma, parent1Fitness, parent2, parent2Sigma, parent2Fitness):
    randInt = np.random.randint(1)

    if randInt == 0:
        return parent1, parent1Sigma, parent1Fitness
    else:
        return parent2, parent2Sigma, parent2Fitness
...
def mutationES(child, childSigma, childFitness, popSize, l, w, maxCuts, zMin, zMax, tMin, tMax, aMin, aMax, res=6):
    ...
    for i in range(len(paramList)):
        ...
        if (i % 3) == 0:
            ...
            if (tempZ < zMiz) or (tempZ > zMax):
                tempZ = paramList[i] - newSigma*norm
            
            paramList[i] = tempZ
        elif (i % 3) == 1:
            tempT = paramList[i] + newSigma*norm
            if (tempT < tMiz) or (tempT > tMax):
                tempT = paramList[i] - newSigma*norm
            
            paramList[i] = tempT
        elif (i % 3) == 2:
            tempA = paramList[i] + newSigma*norm
            if (tempA < aMiz) or (tempA > aMax):
                tempA = paramList[i] - newSigma*norm
...
def getLambdaGeneration(population, popSigmas, popFitness, l, w, sapm, cec, df, latitude, longitude, currFitCount, maxCuts,
                     zMin, zMax, tMin, tMax, aMin, aMax, res=6, conflictDeduction=50, zThresh=20, tThresh=90, aThresh=45):
    ...
    for i in range(7*popSize):
        ...
        parent1Fitness = popFitness[p1_randInt]

        parent2 = population[p2_randInt]
        parent2Sigma = popSigmas[p2_randInt]
        parent2Fitness = popFitness[p2_randInt]


        #recombination
        child, childSigma, childFitness = recombination(parent1, parent1Sigma, parent1Fitness, parent2, parent2Sigma, parent2Fitness)

        #mutation
        newChild, newChildSigma = mutation(child, childSigma, childFitness, popSize, l, w, maxCuts, 
                                    zMin, zMax, tMin, tMax, aMin, aMax, res)

        #geting number of conflicts for newChild
        numConflicts = conflictCount(individual, l, w, zMin, zMax, tMin, tMax, aMin, aMax, res, zThresh, tThresh, aThresh)

        #computeing newChild Fitness
        newChildFitness = getFitness(newChild, numConflicts, l, w, sapm, cec, df, latitude, longitude, currFitCount, res, conflictDeduction)
    ...
    return lambdaGeneration, lambdaSigma, lambdaFitness
